...
export class PhotoService {
  ...
  constructor(
    ...
    private authService: AuthService
      ...
      this.collectionCosasLindas = angularFirestore.collection<Foto>(this.dbPathCosasLindas, ref => ref.orderBy('hora','desc'));
      this.collectionCosasFeas = angularFirestore.collection<Foto>(this.dbPathCosasFeas, ref => ref.orderBy('hora','desc'));
  ...
  async uploadImageAndCreate(item:any, file: any,  dbPath:string):Promise<any>{
    ...
    const upload = await uploadString(imgRef, file['dataUrl'], 'data_url').then((snapshot) => {
  ...
  {
    return this.angularFirestore.collection(dbPath).doc(foto.id).update({likes:foto.likes});
  }
